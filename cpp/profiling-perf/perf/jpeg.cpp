...
Picture random_picture() {
  ...
  Picture picture;
  picture.resize(HEIGHT, std::vector<Pixel>(WIDTH));
  for (size_t i = 0; i < HEIGHT; i++) {
    auto& cur_row = picture[i];
    for (size_t j = 0; j < WIDTH; j++) {
      cur_row[j] = Pixel{color_dist(rng), color_dist(rng), color_dist(rng)};
    }
  }
  return picture;
}
...
Picture scale_down(const Picture& picture) {
  Picture scaled;
  size_t width = (picture[0].size() / 2) + 1;
  scaled.resize((picture.size() / 2) + 1, std::vector<Pixel>(width));

  for (size_t i = 0; i < picture.size(); i += 2) {
    // auto& cur_row = scaled[i];
    for (size_t j = 0; j < picture[0].size(); j += 2) {
      ...
      scaled[i / 2][j / 2] = Pixel{red, blue, green};
    }
  }
