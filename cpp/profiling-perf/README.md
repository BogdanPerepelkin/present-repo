Приложил файл Lab2.pdf, там отчет
...
**Важно:**
В описании MR оставьте максимально подробный отчёт о том, как именно искали и нашли проблему в каждом случае, и как пришли к методу её исправления.
Приложите скриншоты релевантных результатов использованных профилировщиков. Поясните их.
...
- `jpeg` &mdash; найти bottleneck и ускорить бенчмарк в 6+ раз.
- `pi` &mdash; найти bottleneck и ускорить бенчмарк в 50+ раз.
- `radix_sort` &mdash; найти bottleneck и ускорить бенчмарк на 40%+.
- `json` &mdash; понять, что тормозит, и ускорить бенчмарк в 2+ раза, не меняя сами структуры.
- `logger` &mdash; найти bottleneck и ускорить бенчмарк в 25+ раз. Смысл класса `Logger` должен остаться прежним. Содержимое лога должно остаться таким же, как и до ваших изменений.

Профилировать нужно с пресетом `RelWithDebInfo`.
Не забудьте выбрать его, если используете IDE.
...
Можно использовать и другие профилировщики, однако в этой секции будет рассмотрен лишь сценарий использования `perf`.
...
Чуть подробнее о них почитать можно, например, [здесь](https://www.jetbrains.com/help/clion/cpu-profiler.html#Prerequisites).
...
Чтобы собрать руками, можно выполнить следующие команды:

```sh
cmake --preset RelWithDebInfo -B build/RelWithDebInfo -S .
cmake --build build/RelWithDebInfo
```

Бинари для каждой из подзадач будут лежать в `build/RelWithDebInfo/perf`, дальше их можно использовать для запуска под `perf`.
