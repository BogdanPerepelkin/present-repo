...
struct ThrowingMoveAssignmentWithoutMoveConstructor {
  ...
  ThrowingMoveAssignmentWithoutMoveConstructor(ThrowingMoveAssignmentWithoutMoveConstructor&&) noexcept(false) {
    throw std::exception();
  }

  ThrowingMoveAssignmentWithoutMoveConstructor& operator=(ThrowingMoveAssignmentWithoutMoveConstructor&&) = default;
};

void swap(ThrowingMoveAssignmentWithoutMoveConstructor&, ThrowingMoveAssignmentWithoutMoveConstructor&) noexcept {
...
struct MoveConstructorWithoutMoveAssignment {
  ...
  MoveConstructorWithoutMoveAssignment(int val) noexcept
      : x(val) {}

  MoveConstructorWithoutMoveAssignment(const MoveConstructorWithoutMoveAssignment& other) = delete;
  MoveConstructorWithoutMoveAssignment& operator=(const MoveConstructorWithoutMoveAssignment& other) = delete;
  MoveConstructorWithoutMoveAssignment& operator=(MoveConstructorWithoutMoveAssignment&& other) = delete;

  friend void swap(MoveConstructorWithoutMoveAssignment& lhs, MoveConstructorWithoutMoveAssignment& rhs) noexcept {
...
struct NonTrivialIntWrapper {
  ...
  friend constexpr bool operator==(const NonTrivialIntWrapper& lhs, const NonTrivialIntWrapper& rhs) noexcept {
    return lhs.x == rhs.x;
  }
  ...
  friend constexpr auto operator<=>(const NonTrivialIntWrapper& lhs, const NonTrivialIntWrapper& rhs) noexcept {
    return lhs.x <=> rhs.x;
  }
...
struct CoinWrapper {
  ...
  constexpr explicit CoinWrapper(Coin) noexcept
      : coin{17} {}

  constexpr CoinWrapper(const CoinWrapper& other) noexcept
      : coin(other.coin + 1) {}
...
struct OverloadedAddressOf {
  OverloadedAddressOf* operator&() {
    return nullptr;
  }

  const OverloadedAddressOf* operator&() const {
    return nullptr;
  }
...
struct EmptyComparable {
  ...
  EmptyComparable(EmptyComparable&&) {
    throw std::exception();
  }

  EmptyComparable& operator=(EmptyComparable&&) {
    throw std::exception();
  }

  bool operator==(const EmptyComparable&) const {
    throw std::exception();
  }
...
struct CustomComparable {
  CustomComparable(int value, ComparisonCounters* counters)
      : value(value)
      , counters(counters) {}
...
struct CustomOrdered {
  bool operator==(const CustomOrdered&) const {
    return true;
  }

  Ordering operator<=>(const CustomOrdered&) const noexcept {
...
struct ThrowingMembers {
  ...
  ThrowingMembers(const ThrowingMembers&) noexcept {
    ++copy_ctor_calls;
  }
  ...
  ThrowingMembers(ThrowingMembers&&) noexcept {
    ++move_ctor_calls;
  }
