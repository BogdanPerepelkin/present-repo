...
TEST_CASE("[Randomized] - Comparison") {
  ...
  size_t total = 40000;
  std::mt19937 e(seed);
  std::vector<uint32_t> lefts(total);
  std::vector<uint32_t> rights(total);
...
TEST_CASE("[Randomized] - Compare to 2 maps") {
  ...
  Bimap<int, int> b;
  std::map<int, int> left_view;
  std::map<int, int> right_view;
