# 1 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-eef59a\\input\\CompileSource"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 404 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-eef59a\\input\\CompileSource" 2
# 49 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-eef59a\\input\\CompileSource"
typedef float2 FLN;
...
mul(__global MatrixType* srcA, __global MatrixType* srcB, __global MatrixType* dstC, DeviceInfo dims) {
  ...
  __local FLN aTile[16][(6 / 2)];
  __local FLN bTile[(6 / 2)][2][(96 / 2)];
  ...
  const uint TILES_PER_ELEMENT = potolok(dims.kDim, 6);
  for (uint curTileIdx = 0; curTileIdx < TILES_PER_ELEMENT; curTileIdx++) {
    const uint groupShiftK = curTileIdx * 6;
# 133 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-eef59a\\input\\CompileSource"
    const uint groupShiftKV = curTileIdx * (6 / 2);
    const uint aTileSize = 16 * (6 / 2);
    for (uint tileIdx = THREAD_ID; tileIdx < aTileSize; tileIdx += THREADS_IN_GROUP) {
      const uint y = tileIdx / (6 / 2);
      const uint xVec = tileIdx % (6 / 2);
