# 1 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-5e9e6a\\input\\CompileSource"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 404 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-5e9e6a\\input\\CompileSource" 2
# 49 "C:\\Users\\SKKV_S~1\\AppData\\Local\\Temp\\comgr-5e9e6a\\input\\CompileSource"
typedef float2 FLN;
...
kernel void mul(__global MatrixType* srcA, __global MatrixType* srcB, __global MatrixType* dstC, DeviceInfo dims) {
  ...
  for (uint curTileIdx = 0; curTileIdx < TILES_PER_ELEMENT; curTileIdx++) {
    ...
    for (uint y = 0; y < 16; y++) {
      ...
      for (uint xVec = THREAD_ID; xVec < (6 / 2); xVec += THREADS_IN_GROUP) {
