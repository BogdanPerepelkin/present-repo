/* Compiler options:
-c -emit-llvm -target amdgcn-amd-amdhsa -x cl -O3 -cl-kernel-arg-info -Iinclude -DTILE_SIZE_W=96 -DTILE_SIZE_H=16 -DTILE_SIZE_K=66 -DTHREAD_RANGE_W=6 -DTHREAD_RANGE_H=8 -DLOCAL_GR_SIZE_W=16 -DLOCAL_GR_SIZE_H=2 -DN_SIZE=2 -D__OPENCL_VERSION__=200 -D__IMAGE_SUPPORT__=1 -Xclang -cl-ext=+cl_khr_fp64,+cl_khr_global_int32_base_atomics,+cl_khr_global_int32_extended_atomics,+cl_khr_local_int32_base_atomics,+cl_khr_local_int32_extended_atomics,+cl_khr_int64_base_atomics,+cl_khr_int64_extended_atomics,+cl_khr_3d_image_writes,+cl_khr_byte_addressable_store,+cl_khr_fp16,+cl_khr_gl_sharing,+cl_amd_device_attribute_query,+cl_amd_media_ops,+cl_amd_media_ops2,+cl_khr_d3d10_sharing,+cl_khr_d3d11_sharing,+cl_khr_dx9_media_sharing,+cl_khr_image2d_from_buffer,+cl_khr_subgroups,+cl_khr_gl_event,+cl_khr_mipmap_image,+cl_khr_mipmap_image_writes,+cl_amd_copy_buffer_p2p,+cl_amd_planar_yuv -mllvm -amdgpu-prelink -mcode-object-version=5  -include opencl-c.h 
Hash to override:
  Source: 0xf912e18e9e8dccde
  Source + clang options: 0xd4b02570f6a4b837
*/
...
mul(__global MatrixType* srcA, __global MatrixType* srcB, __global MatrixType* dstC, DeviceInfo dims) {
  ...
  for (uint curTileIdx = 0; curTileIdx < TILES_PER_ELEMENT; curTileIdx++) {
    ...
    // TILE A
    // const uint groupShiftKV = curTileIdx * VECTOR_TILE_SIZE_K;
    // const uint aTileSize = TILE_SIZE_H * VECTOR_TILE_SIZE_K;
    // for (uint tileIdx = THREAD_ID; tileIdx < aTileSize; tileIdx += THREADS_IN_GROUP) {
    //   const uint y = tileIdx / VECTOR_TILE_SIZE_K;
    //   const uint xVec = tileIdx % VECTOR_TILE_SIZE_K;
    //   const uint absY = GROUP_SHIFT_H + y;
    //   const uint absX = groupShiftKV + xVec;
    //   FLN packed = (FLN) (0);
    //   if (absY < dims.mDim && absX < kDimVec) {
