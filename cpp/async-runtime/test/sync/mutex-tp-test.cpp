...

  {
    ...
    coro::go(pool, [&](this auto) -> coro::Coroutine {
      for (int j = 0; j < CS_NUM; ++j) {
        ...
        coro::go(pool, [&](this auto) -> coro::Coroutine {
          // std::cout << "lock before " << j << std::endl;
          co_await mutex.lock();
          // std::cout << "lock after " << j << std::endl;
          ++cs;
          // std::cout << "unlock before " << j << std::endl;
          co_await mutex.unlock();
          // std::cout << "unlock after " << j << std::endl;
          wg.done();
        });
      }
      wg.done();
