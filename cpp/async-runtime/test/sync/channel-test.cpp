...
TEST(ChannelTest, SuspendSender) {
  ...
  coro::go(loop, [&](this auto) -> coro::Coroutine {
    ...
    // из очереди прочитал, место освободилось, пробудил sender-а, чтобы положить в очередь T
    // SenderList - 0

    co_await coro::yield();
    // сендер дозавершился под управлением шедулера
